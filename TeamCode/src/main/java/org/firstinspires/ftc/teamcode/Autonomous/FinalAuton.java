...
public class FinalAuton extends AutonomousControl
{
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        if (doingCarousel) {
            rob.driveTrainEncoderMovement(speed, 6, timeoutS, waitAfter, movements.forward);
            rob.driveTrainEncoderMovement(speed, 12, timeoutS, waitAfter, movements.right);
            rob.driveTrainEncoderMovement(speed, 24, timeoutS, waitAfter, movements.ccw);
            rob.runCarouselTimeSpeed(1, 1500);
            rob.driveTrainEncoderMovement(speed, 12, timeoutS, waitAfter, movements.backward);
            rob.driveTrainEncoderMovement(speed, 6, timeoutS, waitAfter, movements.ccw);
            rob.driveTrainEncoderMovement(speed, 48, timeoutS, waitAfter, movements.backward);
            rob.driveTrainEncoderMovement(speed, 28, timeoutS, waitAfter, movements.ccw);
        }
        else {
        ...
        rob.driveTrainEncoderMovement(speed, 24, timeoutS, waitAfter, movements.right);
    }
}